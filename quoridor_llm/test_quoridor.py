...
class TestQuoridorGame:
    def test_game_initialization(self):
        ...
        assert game.players[0].wall_balance == constants.PLAYER_WALL_BALANCE_START
        assert game.players[1].wall_balance == constants.PLAYER_WALL_BALANCE_START

        # Check that no walls are placed initially for both edge types
        for row in range(constants.BOARD_SIZE - 1):
            for col in range(constants.BOARD_SIZE):
                assert not game.edges_up(row, col)

        for row in range(constants.BOARD_SIZE):
            for col in range(constants.BOARD_SIZE - 1):
                assert not game.edges_right(row, col)
    ...
    def test_valid_moves(self):
        ...
        for i in range(3):
            is_ok, message = game.move("A", Dir.RIGHT)
            assert is_ok
            assert message == ""
            assert game.players[0].pos == Pos(5, start_col + i + 1)
        for i in range(3):
            is_ok, message = game.move("A", Dir.LEFT)
            assert is_ok
            assert message == ""
    ...
    def test_invalid_moves_boundaries(self):
        ...
        for _ in range(constants.BOARD_SIZE // 2):
            game.move("B", Dir.RIGHT)

        # Try to move beyond the right boundary
        is_ok, message = game.move("B", Dir.RIGHT)
        assert not is_ok
        assert "boundaries" in message.lower()
    ...
    def test_invalid_moves_walls(self):
        ...
        game = GameState.new_game()
        start_row, start_col = game.players[0].pos

        # Let's place walls so that if the player tries to move up once or left twice they'll
        # face an error
        game.wall_place(start_row, start_col, Dir.UP)
        game.wall_place(start_row, start_col - 1, Dir.LEFT)
    ...
    def test_player_collision(self):
        ...
        for _ in range(constants.BOARD_SIZE // 2):
            is_ok, _ = game.move("A", Dir.UP)
            assert is_ok

        for _ in range(constants.BOARD_SIZE // 2 - 1):
            is_ok, _ = game.move("B", Dir.DOWN)
            assert is_ok
    ...
    def test_wall_placement(self):
        ...
        # Place a wall
        game.wall_place(2, 3, Dir.DOWN)

        # Verify wall is placed two different ways
        assert game.wall_get(2, 3, Dir.DOWN)
        assert game.wall_get(1, 3, Dir.UP)

        with pytest.raises(AssertionError):
            game.wall_place(1, 3, Dir.UP)
